        ...
        <table class="table table-bordered table-striped table-hover align-middle">
            ...
            @foreach($indicators as $indicator)
                <tr>
                    ...
                    <td>{{ $indicator->title_kk }}</td>
                    <td>{{ $indicator->title_en }}</td>
                    <td>{{ $indicator->status }}</td>
                    <td>{{ $indicator->value }}</td>
                    <td>{{ $indicator->unit }}</td>
                    <td>{{ $indicator->unit_kk }}</td>
                    <td>{{ $indicator->unit_en }}</td>
                    <td class="text-end">
                        ...
                        <form action="{{ route('admin.indicators.destroy', $indicator) }}" method="POST" style="display:inline-block;">
                            @csrf
                            @method('DELETE')
                            <button style="border: none; cursor: pointer" onclick="return confirm('Удалить индикатор?')">
                                <svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
